...
fn to_session_error(e: crate::api::groups::GroupError) -> SessionError {
...
async fn find_invite_message(state: &ApiState, invite_id: i32) -> Result<Option<(i32, i64)>, SessionError> {
	use entities::{group_messages, prelude::*};
...
async fn notify_invite_message_status(
	...
	let Some((group_id, message_id)) = find_invite_message(state, invite_id).await? else {
		return Ok(());
	};
...
async fn accept(
	...
	notify_invite_message_status(&state, invite.id, SessionInviteStatus::Accepted).await?;
...
async fn decline(
	...
	notify_invite_message_status(&state, invite.id, SessionInviteStatus::Declined).await?;
